...
C.tmb <- ggs(out.full, family="C")
nu.tmb <- ggs(out.full, family="nu.bar")
mu.tmb <- ggs(out.full, family="mu")

C.vals <- subset(C.tmb, Parameter=="C[4]")$value
nu.vals <- subset(nu.tmb, Parameter=="nu.bar[4]")$value
...
tmb.plot2a <- ggplot(data.fitted, aes(x=fake.P))+
  ...
  theme_classic()+
  ggtitle("TMB")+
  scale_y_continuous(limits=c(min(sub$P.Inf),.75),breaks=c(0,.3,.6))+
