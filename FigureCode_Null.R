############################################
# A FEW DIFFERENT PLOTS for the null model #
############################################
...
for(i in 1:4){
  ...
  nu.vals <- subset(nu.null.data, Parameter==paste("nu.null[", i, "]", sep=""))$value
  mu.vals <- subset(mu.null.data, Parameter==paste("mu.null[", i, "]", sep=""))$value
  ...
  fit.0D <- 1 - exp( -nu.mode * fake.P * 7 )
  fit.3D <- 1 - exp( (-nu.mode * fake.P * 7 ) * exp(-(mu.mode) * 3) )
...
nu.null.data <- ggs(out, family="nu")
mu.null.data <- ggs(out, family="mu")
...
for(i in 1:4){
  ...
  nu.vals <- subset(nu.null.data, Parameter==paste("nu[", i, "]", sep=""))$value
  mu.vals <- subset(mu.null.data, Parameter==paste("mu[", i, "]", sep=""))$value
