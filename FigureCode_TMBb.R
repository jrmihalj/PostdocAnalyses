...
nu.tmb <- ggs(out.noCV, family="nu")
mu.tmb <- ggs(out.noCV, family="mu")

nu.vals <- subset(nu.tmb, Parameter=="nu[4]")$value
mu.vals <- subset(mu.tmb, Parameter=="mu[4]")$value

nu.mode <- NULL; mu.mode <- NULL;
nu.mode <- median(nu.vals)
mu.mode <- median(mu.vals)

fake.P <- seq(0, 0.05, length.out=500)
fit.0D.mean <- 1 - exp( -nu.mode * fake.P * 7 )
fit.3D.mean <- 1 - exp( -nu.mode * fake.P * 7* exp(-(mu.mode) * 3) )
...
for(j in 1:n.iter){
  nu.temp <- sample(nu.vals,1); mu.temp <- sample(mu.vals,1)
  fit.0D[,j] <- 1 - exp( -nu.temp * fake.P * 7 )
  fit.3D[,j] <- 1 - exp( -nu.temp * fake.P * 7* exp(-(mu.temp) * 3) )
}
...
sub <- NULL
sub <- subset(Prop.Inf, P.Inf < 1 &P.Virus=="TMB")

tmb.plot1a <- ggplot(data.fitted, aes(x=fake.P))+
  geom_line(aes(y=fit.0Dmean),linetype=5, size=1, color="gray")+
...
legend(x=c("topleft"), legend=c("0 Day", "3 Day"), pch=c(19,17), pt.cex=c(1.25,1.25), 
       lty=c(5,1), lwd=c(2,2), col=c("gray","black"), bty="n")


tmb.plot1b <- ggplot(data.fitted, aes(x=fake.P))+
...
tmb.plot2a <- ggplot(data.fitted, aes(x=fake.P))+
  ...
  theme_classic()+
  ggtitle("TMB")+
  scale_y_continuous(limits=c(min(sub$P.Inf),.65),breaks=c(0,.3,.6))+
...
tmb.plot2b <- ggplot(data.fitted, aes(x=fake.P))+
  geom_segment(data=errs, aes(y=mean.y-se.y, yend=mean.y+se.y, x=mean.x, xend=mean.x),width=.5)+
  geom_segment(data=errs, aes(y=mean.y, yend=mean.y, x=mean.x-se.x, xend=mean.x+se.x),width=.5)+
  geom_point(data = errs[1:3,], aes(x = mean.x, y = mean.y), shape=19, size=3, color="gray")+
  geom_point(data = errs[4:6,], aes(x = mean.x, y = mean.y), shape=17, size=3)+
  ...
  theme_classic()+
  ggtitle("TMB")+
  scale_y_continuous(limits=c(0,1.05),breaks=c(0,.5,1))+
  scale_x_continuous(limits=c(0,0.035), breaks=c(0,0.01,0.03))+
  #theme(legend.position="none")+
  ylab("-ln(1 - Prop.Inf.)") + xlab(expression(paste("Cad. Density (cadavers*", cm^-2, ")")))
