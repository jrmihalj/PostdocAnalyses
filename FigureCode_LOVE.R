...
nu.love <- ggs(out.noMu, family="nu")
nu.love <- subset(nu.love, Parameter=="nu.bar[2]")$value
nu.love.mode <- median(nu.love)

C.love <- ggs(out.noMu, family="C")
C.love <- subset(C.love, Parameter=="C[2]")$value
C.love.mode <- median(C.love)

fake.P <- seq(0, 0.05, length.out=500)
fit.mean <- 1 - (1 + C.love.mode^2 * nu.love.mode * fake.P * 7)^(-1/(C.love.mode^2))

# Bootstrap storage:
n.iter <- 1000
fit.love <- mat.or.vec(nr=length(fake.P), nc=n.iter)

for(j in 1:n.iter){
  nu.temp <- sample(nu.love,1); C.temp <- sample(C.love,1);
  fit.love[,j] <- 1 - (1 + C.temp^2 * nu.temp * fake.P * 7)^(-1/(C.temp^2))
}
...
for(j in 1:length(fake.P)){
  ...
  fit.range[j,2:3] <- as.vector(quantile(fit.love[j,], prob=c(0.025,0.975))[1:2])
...
sub <- NULL
sub <- subset(Prop.Inf, P.Inf<1 &P.Virus=="LOVE") #Subset out love

love.plot1a <- ggplot(data.fitted, aes(x=fake.P))+
...
love.plot1b <- ggplot(data.fitted, aes(x=fake.P))+
...
love.plot2a <- ggplot(data.fitted, aes(x=fake.P))+
  geom_line(aes(y=fit.mean), linetype=1, size=1)+
  geom_line(aes(y=fit.low), linetype=1, size=.85, color="gray")+
  geom_line(aes(y=fit.hi), linetype=1, size=.85, color="gray")+
...
love.plot2b <- ggplot(data.fitted, aes(x=fake.P))+
  geom_line(aes(y=-log(1-fit.mean)), linetype=1, size=1)+
